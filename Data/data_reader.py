    ...
    for file in data_files:
        ...
        for i in range(2, 6):  # first four are heights
            for j in range(6, 10):  # second four are radii
                ftr['ftr{}'.format(j - 2) + '/' + 'ftr{}'.format(i - 2)] = ftr.apply(
                    lambda row: row.iloc[j] / row.iloc[i], axis=1)
                newCol += 1
    ...
    for file in files_to_filter:
        with open(file, 'r') as f:
            ...
            print('{} reduced from {} to {}, ({}%)'.format(file, lengthsPreFilter[-1],
                                                           lengthsPostFilter[-1],
                                                           100 * np.round(lengthsPostFilter[-1] / lengthsPreFilter[-1],
                                                                          4)))
...
def split_data_train_test(data_x, data_y, test_ratio, rand_seed):
...
def get_data_into_loaders(data_x, data_y, batch_size, DataSetClass,
                          rand_seed=1234, test_ratio=0.05, subsample=None):
    """
    ...
    if subsample is not None:
        rand_permutation = np.random.permutation(len(x_train))
        x_train = x_train[rand_permutation][:subsample]
        y_train = y_train[rand_permutation][:subsample]
    print("Total number of training samples is: {}".format(len(x_train)))
    print("The dimension of the features is: {}".format(len(x_train[0])))
    print('total number of test sample is {}'.format(len(y_test)))
...
def read_data_sine_test_1d(num_points = "32768", eval_data_all=False):
    """
    Data reader function for testing sine wave 1d data (1d to 1d)
    :param flags: Input flags
    """
    data_dir = "Data/1D_Sinusoid/"
    if eval_data_all is True:
        data_dir = os.path.join(data_dir, "evaluation_set/")
    else:
        data_dir = os.path.join(data_dir, "{}_points/".format(num_points))

    x_filename = os.path.join(data_dir, "data_x.csv")
    data_x = pd.read_csv(x_filename, header=None).astype('float32').values
    y_filename = os.path.join(data_dir, "data_y.csv")
    data_y = pd.read_csv(y_filename, header=None).astype('float32').values
    assert len(data_x) == len(data_y)
    num_examples = len(data_x)
    batch_size = int(num_examples / 8)
    if eval_data_all:
        return get_data_into_loaders(data_x, data_y, batch_size,
                                     SimulatedDataSet_class_1d_to_1d,
                                     test_ratio=0.99)
    return get_data_into_loaders(data_x, data_y, batch_size,
                                 SimulatedDataSet_class_1d_to_1d,
                                 test_ratio=0.2)

def get_sine_wave_train_eval_data_arrays():
...
def read_data_sine_wave(eval_data_all=False, test_ratio=0.2):
    """
    ...
    if eval_data_all:
        return get_data_into_loaders(data_x, data_y, 1024,
                                     SimulatedDataSet_class, test_ratio=0.999)
    return get_data_into_loaders(data_x, data_y, 1024,
                                 SimulatedDataSet_class, test_ratio=test_ratio)

def get_bens_sine_wave_data(eval_data_all=False, test_ratio=0.2,
                            subsample=None):
    data_dir = "/home/gps10/Documents/BDIMNNA/Simulated_DataSets/"
    data_dir = os.path.join(data_dir, "Sinusoidal_Wave/")
    if eval_data_all is True:
...
def read_data_robotic_arm(eval_data_all=False, test_ratio = 0.2,
                          subsample=None):
    """
    ...
    """
    data_dir = "/home/gps10/Documents/BDIMNNA/Simulated_DataSets/"
    data_dir = os.path.join(data_dir, "Robotic_Arm/")
    ...
    if eval_data_all:
        return get_data_into_loaders(data_x, data_y, 1024,
                                     SimulatedDataSet_regress, test_ratio=0.999)
    return get_data_into_loaders(data_x, data_y, 1024,
                                 SimulatedDataSet_regress,
                                 test_ratio = test_ratio,
                                 subsample=subsample)

def read_data_ballistics(eval_data_all=False, test_ratio=0.2, data_dir=None,
                         no_poisson=False, uniform=False):
    """
    ...
    """
    data_dir = "/home/gps10/Documents/BDIMNNA/Simulated_DataSets/"
    data_dir = os.path.join(data_dir, 'Ballistics/')
    if no_poisson is True:
        data_dir = os.path.join(data_dir, "No_Poisson")
    if uniform is True:
        data_dir = os.path.join(data_dir, "Uniform")        
    """
    if eval_data_all is True:
        data_dir = os.path.join(data_dir, "Test")
...
def read_data_ensemble_MM(eval_data_all=False, test_ratio=0.2):
    data_dir = "/home/gps10/Documents/BDIMNNA/Simulated_DataSets/"
    data_dir = os.path.join(data_dir, "Meta_material_Neural_Simulator/")
    data_dir = os.path.join(data_dir, "dataIn")
    if eval_data_all is True:
        data_dir = os.path.join(data_dir, "Test")
    x_filename = os.path.join(data_dir, "data_x.csv")
    data_x = pd.read_csv(x_filename, header=None, sep=' ').astype('float32')
    data_x = data_x.values
    y_filename = os.path.join(data_dir, 'data_y.csv')
    data_y = pd.read_csv(y_filename, header=None, sep=' ').astype('float32')
...
def read_data_peurifoy(data_dir="/home/gps10/Documents/AEM_DIM_Bench/Data/",
                       batch_size=1024, test_ratio=0.2, eval_data_all=False):
    """
    Data reader function for the Peurifoy generated data set
    :param flags: Input flags
    :return: train_loader and test_loader in pytorch data set format (normalized)

    Defaults have been ascertained by Greg from Ben's training code -- formerly flags
    """

    # Read the data
    data_dir = os.path.join(data_dir, 'Peurifoy')
    if test_ratio > 0.2:
        data_dir = os.path.join(data_dir, "Test/")
    print(f"Loading Peurifoy data from: {data_dir}")
    x_filepath = os.path.join(data_dir, "data_x.csv")
    data_x = pd.read_csv(x_filepath, header=None).astype('float32').values
    y_filepath = os.path.join(data_dir, 'data_y.csv')
    data_y = pd.read_csv(y_filepath, header=None).astype('float32').values
    ...
    if eval_data_all:
        return get_data_into_loaders(data_x, data_y, batch_size,
                                     SimulatedDataSet_regress, test_ratio=0.8) #0.999
